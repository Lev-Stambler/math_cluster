...
def main():
	...
	print("Theorem numbs: {}".format(len(thms)))
	model = T5EncoderModel.from_pretrained("kaiyuy/leandojo-lean3-retriever-byt5-small")
	chunk_size = 64
	rets = []
	for i in range(0, len(thms), chunk_size):
		thm_chunk = thms[i:min(i+chunk_size, len(thms))]
		thm_chunk = [t.ast.text.split(":=")[0].split("\n|")[0].strip() for t in thm_chunk]
		embeddings = encode(model, thm_chunk)
		print(embeddings.shape)
		emb_list = [(thm_chunk[i], embs) for i, embs in enumerate(embeddings.tolist())]

		rets += emb_list
		json.dump(rets, open("data_store/embeddings.json", "w"))
	# TODO: write to JSON
